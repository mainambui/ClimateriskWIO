...
# df.hzd <- as.data.frame(hazards, xy=TRUE)
#Quantile transform
# m1 <- names(hazards)
...
df2 <- cbind("ID" = climdata[,"ID"], sce = "ssp585", climdata[,namelist[grep("_585_2050", namelist)]]); colnames(df2) <- c("ID","Scenario", paste(varlst))

dfNormed <- rbind(df1,df2) %>% as.data.frame() %>% mutate(pH_trend = -1*pH_trend)%>% mutate(across(cdd:sst90Int, ~ inormal(.x)))
...
(dfNormedWide <- lapply(1:length(varlst), FUN = function(x){
  ...
  names(xv) <- c("ID", paste(varlst[[x]], c("ssp245","ssp585"), sep = "_"))
  return(xv)
}))
...
write_csv(ClimImpacts, "2_Data/sheet/2_ClimateImpacts.csv")
