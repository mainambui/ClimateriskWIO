...
climdataNew <- extract(stack(mn_stk,sd_stk), wio.AOO.spdf, sp=TRUE,df=TRUE) %>% as.data.frame()
colnames(climdataNew)[colnames(climdataNew) == "coords.x1"] <- "x"
colnames(climdataNew)[colnames(climdataNew) == "coords.x2"] <- "y"

#Calculate Exposure Dimensions of the RISK model.
names(climdataNew)
grdArea = (25e3)^2
(climdataNew <- climdataNew %>% 
...
hist(climdataNew$imp.ssp370.2050, breaks=30)
write_rds(climdataNew, "2_Data/spreadsheet/1_Climate/wioAOO.metrics.rds")
